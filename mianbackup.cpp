...
public:
    ...
    explicit game(const string &g) : gamemode(g), gametable(g), playerturn("white"), tileselected(nullptr),pieceselected(nullptr),piecetaken(nullptr) {
    };
    ...
    void redo_invalid_move() {
        pieceselected->setTilepair(make_pair(string_previous_tileselected, previous_tileselected));
        if (piecetaken != nullptr) {
            //cout << piecetaken << " " << piecetaken->getPiececolor() << endl;
            tileselected->setontile(piecetaken->getPiececolor());
            table::restorepiece(piecetakenname, piecetaken);
        } else {
            tileselected->setontile("empty");
        }
        previous_tileselected->setontile(pieceselected->getPiececolor());
